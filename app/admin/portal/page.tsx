...
export default async function AdminPortal() {
    ...
    return (
        <div className="min-h-screen bg-gray-950 text-white font-sans flex flex-col items-center py-20 px-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    {allowedBots.map((bot) => (
                        <Link
                            key={bot.id}
                            href={`/admin/${bot.id}`}
                            className="group relative bg-gray-900 border border-gray-800 rounded-2xl p-6 hover:border-indigo-500/50 hover:bg-gray-800/50 transition-all"
                        >
                            <div className="w-12 h-12 bg-gray-800 rounded-xl flex items-center justify-center mb-4 group-hover:bg-indigo-500/20 group-hover:text-indigo-400 transition-colors">
                                <CpuChipIcon className="w-6 h-6 text-gray-400 group-hover:text-indigo-400" />
                            </div>
                    ...
                    {isSuperAdmin && (
                        <Link
                            href="/admin/super"
                            className="group relative bg-gradient-to-br from-indigo-900/20 to-purple-900/20 border border-indigo-500/30 rounded-2xl p-6 hover:border-indigo-500/70 hover:bg-indigo-900/30 transition-all border-dashed"
                        >
                    ...
                    {allowedBots.length === 0 && !isSuperAdmin && (
                        <div className="col-span-full py-12 text-center border border-gray-800 border-dashed rounded-2xl">
                            <RectangleStackIcon className="w-12 h-12 mx-auto text-gray-700 mb-4" />
                            <p className="text-gray-500 font-medium">No bots assigned to your account.</p>
                            <p className="text-gray-600 text-sm mt-1">Contact your administrator for access.</p>
                        </div>
                    )}
                </div>
            </div>
