(function () {
    function mount(options) {
        ...
        var existingLauncher = document.getElementById('bizai-agent-launcher');
        var existingIframe = document.getElementById('bizai-agent-iframe');
        if (existingLauncher) existingLauncher.remove();
        if (existingIframe) existingIframe.remove();

        var launcher = document.createElement('button');
        launcher.id = 'bizai-agent-launcher';
        ...
        window.addEventListener('message', function (e) {
            if (e.data && e.data.type === 'BIZAI_RESIZE') {
    ...
    function auto() {
        ...
        if (!script) {
            ...
            for (var i = 0; i < scripts.length; i++) {
                if (scripts[i].src && scripts[i].src.includes('/embed/bizai-agent.js')) {
                    script = scripts[i];
                    break;
                }
            }
        ...
        // Read context from multiple sources (priority: window.BIZAI_AGENT_CONTEXT > data-context attribute)
        var context = null;

        // First, try to read from window.BIZAI_AGENT_CONTEXT (global variable)
        if (window.BIZAI_AGENT_CONTEXT && typeof window.BIZAI_AGENT_CONTEXT === 'object') {
            context = window.BIZAI_AGENT_CONTEXT;
        }
        ...
        if (!context) {
            try {
                ...
                console.error('BizAI Agent: Invalid JSON in data-context attribute', e);
            }
        }
    ...
    window.BizAIAgent = { mount: mount };
    window.LolaBot = { mount: mount }; // Alias for backwards compatibility
